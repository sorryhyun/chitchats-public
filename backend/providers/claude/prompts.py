"""
Claude provider prompt configuration.

This module provides functions to load Claude-specific prompts from prompts.yaml.
"""
...
def get_base_system_prompt() -> str:
    """
    Get the base system prompt for Claude provider.
    ...
    try:
        ...
        logger.warning(f"System prompt '{active_prompt_key}' not found for Claude provider, using fallback")
        return DEFAULT_FALLBACK_PROMPT

    except Exception as e:
        logger.error(f"Error loading Claude system prompt: {e}")
        return DEFAULT_FALLBACK_PROMPT
