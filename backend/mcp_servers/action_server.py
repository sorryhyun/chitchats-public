...
def create_action_server(
    ...
    async def handle_list_tools() -> list[types.Tool]:
        ...
        if is_tool_enabled("skip"):
            skip_config = tool_configs.get("skip", {})
            tools.append(types.Tool(
                name="skip",
                description=skip_config.get(
                    "description",
                    "Skip responding to the current message if you have nothing meaningful to add."
                ),
                inputSchema=SkipInput.model_json_schema(),
            ))

        if is_tool_enabled("memorize"):
            memorize_config = tool_configs.get("memorize", {})
            tools.append(types.Tool(
                name="memorize",
                description=memorize_config.get(
                    "description",
                    "Record a new memory or observation for future reference."
                ),
                inputSchema=MemorizeInput.model_json_schema(),
            ))

        if is_tool_enabled("recall"):
            recall_config = tool_configs.get("recall", {})
            subtitles = list(memory_index.keys())
            desc = recall_config.get(
                "description",
                "Retrieve long-term memories by subtitle."
            )
            if subtitles:
                desc += f" Available subtitles: {', '.join(subtitles)}"
            tools.append(types.Tool(
                name="recall",
                description=desc,
    ...
    async def handle_call_tool(name: str, arguments: dict) -> list[types.TextContent]:
        """Handle tool calls."""
        if name == "skip":
            response_text = get_tool_response("skip", group_name=agent_group)
            ...
            if config_file:
                ...
                try:
                    ...
                    response_text = get_tool_response("memorize", group_name=agent_group)
            ...
            if subtitle in memory_index:
                ...
                response_text = get_tool_response("recall", group_name=agent_group)
...
async def main():
    ...
    logger.info(f"Starting action MCP server for agent: {agent_name}")
