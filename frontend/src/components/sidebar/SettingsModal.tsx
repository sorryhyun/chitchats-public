...
export const SettingsModal = ({ isOpen, onClose }: SettingsModalProps) => {
  ...
  const { useSonnet, modelName, setUseSonnet, loading: modelLoading } = useModelSetting();
      ...
      <ModalContent className="space-y-6">
        ...
        <div className="bg-slate-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Toggle
              checked={useSonnet}
              onCheckedChange={setUseSonnet}
              disabled={modelLoading}
            />
          </div>

          <StatusIndicator
            status={useSonnet ? 'available' : 'disabled'}
            text={modelLoading ? t('modelLoading', 'Loading...') : modelName}
          />

          <p className="mt-3 text-xs text-slate-500">
            {t(
              'modelHelpText',
              'Toggle between Opus (more capable) and Sonnet (faster). Takes effect on the next conversation turn.'
            )}
          </p>
        </div>
